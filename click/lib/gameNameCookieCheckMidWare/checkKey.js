...
function checkKey() {
  ...
  const middleware = (req, res, next) => {
    if (
      ...
      Logger.log({
        status: 500,
        event: "gameNameCookieCheckMidWare/checkKey",
        ob: [res.locals.gameNameCookie, res.locals.dbData],
    ...
    if (!res.locals.gameNameCookie[gameNameHash]) {
      Logger.log({
        status: 500,
        event: "gameNameCookieCheckMidWare/checkKey",
        ob: [res.locals.gameNameCookie, gameNameHash],
        gameName: res.locals.gameName,
        roomCode: res.locals.roomCode,
        message: "hash",
      });
    ...
    if (
      res.locals.gameNameCookie[gameNameHash] !=
